...
CalcSgRNAPropPK <- R6Class(
  ...
  public = list(
    ...
    future.freq.odds.ratio.vec = NULL, # single and dinucleotide frequencies (Freq)
    ...
    initialize = function(fourtybase.region.surrounding.targetseq.vec = character(0),
      ...
      #   align.save.file.char: a connection or the name of the file where the alignment score list is saved to or read from.
      #   script.dir: directory path contains script. 
      ...
      tryCatch({
        ...
        #load class
        source(file.path(self$script.dir, "FormatSgRNAInfo-class.R"))
        ...
        private$MakePDfreqPropertiesTable() # Make pdfreq Properties Table
    ...
    ShowDataValue = function(datatype) {
      ...
      #
      # Args:
      #   datatype: data type {"all"|"pdfreq"|"continuous"}
      #
      # Returns:
      ...
      if(datatype == "all"){
        return(res.df)
      }else if(datatype == "pdfreq"){
        return(res.df[, c(colnames(self$all.pdfreq.properties.df), "label")])
      }else if(datatype == "continuous"){
        return(res.df[, c(colnames(self$all.continuous.properties.df), "label")])
      }else{
        stop("legal argument are only all or pdfreq or continuous. Please check the input.")
      }
    }
  ),
  private = list(
    ...
    calcLogOdds = function(pos.df, neg.df) {
      # calculate log odds ratio
      #
      # Args:
      #   pos.df: positive data frame
      #   neg.df: negative data frame
      #
      # Returns:
      ...
      tryCatch({
        # Error handling
        stopifnot(is.data.frame(pos.df))
        stopifnot(is.data.frame(neg.df))
        
        # Calculate Lod odds ratios
        logodd.vec <- numeric(0)
        pos.vec <- as.data.frame( t( apply(pos.df, 2, sum) ) )
        neg.vec <- as.data.frame( t( apply(neg.df, 2, sum) ) )
        # browser()
        temp.sum.vec <- rbind.fill(pos.vec, neg.vec)
        temp.sum.vec[is.na(temp.sum.vec)] <- 0
        sum.vec <- apply(temp.sum.vec, 2, sum)
        for (property in intersect( names(pos.vec), names(neg.vec) )){
        ...
        names(logodd.vec) <- paste0(names(logodd.vec), ".odds")
        return(logodd.vec)
      }
      , error = function(e) {
        message(e)
    ...
    CalcFutureNN = function() {
      ...
      self$mononuc.high.CalcNNucFreq <- CalcNNucFreq$new(self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("whole", "high"),
                                                              N = 1, exception.vec = c(32, 33))
      message("low sequence set")
      self$mononuc.low.CalcNNucFreq <- CalcNNucFreq$new(self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("whole", "low"),
                                                                N = 1, exception.vec = c(32, 33))
      message("Dinucleotide")
      message("high sequence set")
      self$dinuc.high.CalcNNucFreq <- CalcNNucFreq$new(self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("whole", "high"),
                                                            N = 2, exception.vec = c(32, 33))
      message("low sequence set")
      self$dinuc.low.CalcNNucFreq <- CalcNNucFreq$new(self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("whole", "low"),
                                                              N = 2, exception.vec = c(32, 33))
      message("--------------------------------")
      message("--------------------------------")
    ...
    CalcFutureThermo = function() {
      ...
      message("--------------------------------")
      self$future.thermo.CalcThermo  <- CalcThermo$new(self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "all"), 0.2, self$script.dir, self$ref.dir)
    ...
    CalcFuturePacker = function() {
      ...
      message("--------------------------------")
      self$future.packer.CalcPacker <- CalcPacker$new(self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "all"), self$ref.dir)
    ...
    CalcFuturePhyChem = function() {
      ...
      message("--------------------------------")
      self$future.phychem.CalcPhyChem <- CalcPhyChem$new(self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "all"), self$ref.dir)
      ...
      if(!file.exists(self$align.save.file.char)){
        ...
        for(k in self$align.range.vec){
          ...
          self$future.align.CalcAlign <- CalcAlign$new(
            ...
            len1.num = min( c(k, nchar( self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "all") ) ) ),
            ori1.char = "3' to 5'",
    ...
    MakePDfreqPropertiesTable = function() {
      self$all.pdfreq.properties.df <- rbind(data.frame(
    ...
    MakePropertiesTable = function(){
      self$all.properties.df <- merge(self$all.pdfreq.properties.df,
                                      self$all.continuous.properties.df,
                                      by = "sequence", all=T
      )
      
      self$high.properties.df <- data.frame(sequence = self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "high"),
                                                 stringsAsFactors = FALSE)
      self$low.properties.df <- data.frame(sequence = self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "low"),
                                                   stringsAsFactors = FALSE)
    ...
    MakeContinuousPropertiesTable = function() {
      ...
      # Align
      temp.df <- NULL
      temp.df <- self$future.align.list[[paste0(min(self$align.range.vec), "bp")]]$sequence1
      ...
      self$high.continuous.properties.df <- data.frame(sequence = self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "high"),
                                                            stringsAsFactors = FALSE)
      self$low.continuous.properties.df <- data.frame(sequence = self$sgrnas.info.FormatSgRNAInfo$RetrieveSeqset("proto", "low"),
                                                              stringsAsFactors = FALSE)
