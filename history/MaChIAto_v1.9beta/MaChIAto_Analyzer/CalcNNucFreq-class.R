  ...
  public = list(
    ...
    CalcSeqFreq = function(seqset, exception.vec = numeric(0), N = 1) {
      ...
      tryCatch({
        ...
        for(seq.ind in 1:length(seqset)){
          # Calculate N-nucleotide frequency
          startpos <- 1
          endpos <- startpos + N - 1
          pdfreq <- numeric(0)
          freq <- oligonucleotideFrequency(seqset[seq.ind], width = N) * 0
          repeat{
            
            # check parameters
            if(!(startpos %in% exstartpos.vec)){
              # count frequency
              atfreq <- oligonucleotideFrequency(subseq(seqset[seq.ind], start = startpos, end = endpos), width = N)
              # Calculate N-nucleotide frequency
              freq <- freq + atfreq
            ...
            startpos <- startpos + 1
            endpos <- startpos + N - 1
            if(endpos > width(seqset[seq.ind])[1]) break
            
          }
          if(seq.ind == 1){
            sum.freq <- as.data.frame(freq)
            sum.pdfreq <- as.data.frame(pdfreq)
          }else{
            sum.freq <- rbind.fill( as.data.frame(sum.freq), as.data.frame(freq) )
            sum.pdfreq <- rbind.fill( as.data.frame(sum.pdfreq), as.data.frame(pdfreq) )
          }
        }
        sum.pdfreq[is.na(sum.pdfreq)] <- 0

        res.list = list(sum.freq, sum.pdfreq)
