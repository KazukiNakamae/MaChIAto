...
CalcMicroHomologyPropPK <- R6Class(
  ...
  public = list(
    ...
    is.left.both.microhomology.logical = FALSE,
    ...
    initialize = function(whole.region.microhomology.seq.vec = character(0),
                          is.left.both.microhomology.logical = TRUE,
      ...
      tryCatch({
        ...
        self$microhomology.info.FormatMicroHomologyInfo <- FormatMicroHomologyInfo$new(
          ...
          is.left.both.microhomology.logical,
          scaffold.seq.vec,
  ...
  private = list(
    ...
    calcLogOdds = function(pos.df, neg.df) {
      ...
      #
      # Args:
      #   pos.df: positive matrix
      #   neg.df: negative matrix
      #
      # Returns:
    ...
    CalcFutureAlign = function() {
      ...
      if(!file.exists(self$align.save.file.char)){
        ...
        message("Perform optimal pairwise alignments between the focused sequence and the guide RNA sequence in")
        for(k in self$align.range.vec){
          message(k,"bp region")
          message("-----------------------------")
          # browser()
          if(self$microhomology.info.FormatMicroHomologyInfo$is.left.both.microhomology.logical){
              self$future.align.CalcAlign <- CalcAlign$new(
                  self$microhomology.info.FormatMicroHomologyInfo$RetrieveSeqset("focus", "all"),
                  len1.num = min( c(k,  nchar(self$microhomology.info.FormatMicroHomologyInfo$RetrieveSeqset("focus", "all") ) ) ),
                  ori1.char = "3' to 5'",
              ...
              self$future.align.CalcAlign <- CalcAlign$new(
                  self$microhomology.info.FormatMicroHomologyInfo$RetrieveSeqset("focus", "all"),
                  len1.num = min( nchar(self$microhomology.info.FormatMicroHomologyInfo$RetrieveSeqset("focus", "all") ) ),
                  ori1.char = "5' to 3'",
                  self$microhomology.info.FormatMicroHomologyInfo$RetrieveSeqset("scaffold", "all"),
    ...
    MakePDfreqPropertiesTable = function() {
      self$all.pdfreq.properties.df <- rbind.fill(data.frame(
        surrounding.sequence = self$mononuc.high.CalcNNucFreq$summary$whole.sequence,
        sequence = self$mononuc.high.CalcNNucFreq$summary$whole.sequence,
      ...
      data.frame(
        ...
        sequence = self$mononuc.low.CalcNNucFreq$summary$whole.sequence,
        self$mononuc.low.CalcNNucFreq$summary$PDfrequency,
        self$dinuc.low.CalcNNucFreq$summary$PDfrequency,
        stringsAsFactors = FALSE
      ))
      self$all.pdfreq.properties.df[is.na(self$all.pdfreq.properties.df)] <- 0
    },
    ...
    MakeContinuousPropertiesTable = function() {
      ...
      self$all.continuous.properties.df <- data.frame(
        sequence = c(
          self$mononuc.high.CalcNNucFreq$summary$whole.sequence,
          self$mononuc.low.CalcNNucFreq$summary$whole.sequence
        )
        ,stringsAsFactors = FALSE)
