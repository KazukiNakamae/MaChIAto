...
CalcMHScore <- R6Class(
  ...
  public = list(
    ...
    save.dir = "", # directory which has output files

    # constructor
    initialize = function(seq.vec, cutpos.vec, length_weight = 20.0, script.dir = ".", save.dir = ".") {
    ...
    ProfileMH = function(ind) {
      ...
      tryCatch({
        ...
        res <- system(paste("python", file.path(self$script.dir, "calcMHscore.py"), self$seq.vec[ind], self$length_weight, self$cutpos.vec[ind], self$save.dir), intern = TRUE)
        # print(res)
        ...
        message(paste0( substr(self$seq.vec[ind], 1, self$cutpos.vec[ind]), "//", substr(self$seq.vec[ind], self$cutpos.vec[ind] + 1, nchar(self$seq.vec[ind]))))
        message("Microhomology score")
        message(scores[1])
        message("Out-of-frame score")
        message(scores[2])
