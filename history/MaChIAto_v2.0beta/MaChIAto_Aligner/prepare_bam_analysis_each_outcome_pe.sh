...
# Mutation
awk -F, 'BEGIN{OFS=","}{ gsub ("Mixed HDR-NHEJ", "Mixed", $77); if ($72 == "unidentified" && $77 == class) {for (i=1; i<=$10; i++) print ">"$1"\n"$2} }' class=$DATATYPE $INDIR"/ALL_dataframe.csv" > $DATADIR"/fasta/"$DATATYPE"/other_type/unidentified.fa";
for type in "untreated" "unexpected_mutations" "overall_edited_substitution" "overall_edited_insertion" "overall_edited_deletion";
  do awk -F, 'BEGIN{OFS=","}{ gsub ("Mixed HDR-NHEJ", "Mixed", $77); if ($72 == type && $77 == class) {for (i=1; i<=$10; i++) print ">"$1"\n"$15} }' type=$type class=$DATATYPE $INDIR"/ALL_dataframe.csv" > $DATADIR"/fasta/"$DATATYPE"/mut_type/"$type".fa";
done;
...
for type in "LEFT_PRECISE_KNOCK_IN" "LEFT_IMPRECISE_KNOCK_IN" "LEFT_IMPRECISE_KNOCK_IN_COMPLEX" "LEFT_OTHER_MUATIONS";
  do awk -F, 'BEGIN{OFS=","}{ gsub ("Mixed HDR-NHEJ", "Mixed", $77); if ($73 == type && $60 == "False" && $77 == class) {for (i=1; i<=$10; i++) print ">"$1"\n"$15;} }' type=$type class=$DATATYPE $INDIR"/ALL_dataframe.csv" > $DATADIR"/fasta/"$DATATYPE"/ki_type/"$type".fa";
done;
for type in "LEFT_IMPRECISE_KNOCK_IN" "LEFT_IMPRECISE_KNOCK_IN_COMPLEX" "LEFT_OTHER_MUATIONS";
  do awk -F, 'BEGIN{OFS=","}{ gsub ("Mixed HDR-NHEJ", "Mixed", $77); if ($73 == type && $60 == "True" && $77 == class) {for (i=1; i<=$10; i++) print ">"$1"\n"$15;} }' type=$type class=$DATATYPE $INDIR"/ALL_dataframe.csv" > $DATADIR"/fasta/"$DATATYPE"/rc_ki_type/rc_"$type".fa";
done;
for type in "RIGHT_PRECISE_KNOCK_IN" "RIGHT_IMPRECISE_KNOCK_IN" "RIGHT_IMPRECISE_KNOCK_IN_COMPLEX" "RIGHT_OTHER_MUATIONS";
  do awk -F, 'BEGIN{OFS=","}{ gsub ("Mixed HDR-NHEJ", "Mixed", $77); if ($74 == type && $64 == "False" && $77 == class) {for (i=1; i<=$10; i++) print ">"$1"\n"$15} }' type=$type class=$DATATYPE $INDIR"/ALL_dataframe.csv" > $DATADIR"/fasta/"$DATATYPE"/ki_type/"$type".fa";
done;
for type in "RIGHT_IMPRECISE_KNOCK_IN" "RIGHT_IMPRECISE_KNOCK_IN_COMPLEX" "RIGHT_OTHER_MUATIONS";
  do awk -F, 'BEGIN{OFS=","}{ gsub ("Mixed HDR-NHEJ", "Mixed", $77); if ($74 == type && $64 == "True" && $77 == class) {for (i=1; i<=$10; i++) print ">"$1"\n"$15} }' type=$type class=$DATATYPE $INDIR"/ALL_dataframe.csv" > $DATADIR"/fasta/"$DATATYPE"/rc_ki_type/rc_"$type".fa";
done;
