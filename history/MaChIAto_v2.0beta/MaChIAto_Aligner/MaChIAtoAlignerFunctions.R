...
MakeWtCrisprSet <- function(untreated.seq, index.name, wt.seq, protospacer.seq, bam.fn, names, snv.range){
  wt.crispr.set <- NULL
  ...
  tryCatch({
    ...
    wt.crispr.set <- readsToTarget(bam.fn
                                ...
                                , names = names
                                , upstream.snv = snv.range
                                , downstream.snv = snv.range
                                , chimeras = "ignore"
                                )
    return(wt.crispr.set)
  }, 
...
MakeKiCrisprSet <- function(
  ...
  # browser(expr = (ki.mode == "left.precise"))
  ki.crispr.set <- NULL
  bf <- open(BamFile(bam.fn))
  bam.len <- length(scanBam(bf)[[1]]$qname)
  close(bf)
...
SaveCompBarPlot <- function(count.table, color.code, file.name, xlab, ylab){
  
  if(sum(count.table$Frequency) < 1){
    return()
  }

  options(warn = -1)
  count.p <- ggplot(data=count.table
    , aes(x=Position, y=Frequency)
  ) +
  geom_bar(stat="identity", width=0.5, alpha = 1.0, fill = color.code) +
  coord_cartesian(ylim = c(0, max(count.table$Frequency) + 100)) +
  ...
  xlab(xlab) +
  ylab(ylab) +
  geom_text(aes(label = as.character(count.table$Frequency)), vjust = -0.3)
  # plot(count.p)
  ggsave(file = file.name, plot = count.p, dpi = 300, width = 5, height = 5, limitsize = FALSE)
  options(warn = 0)
...
SaveVariantsData <- function(crispr.set, out.dir, range.vec = -35:35, mut.type){
  ...
  tryCatch({
    ...
    # variants
    variant.count.df <- variantCounts(crispr.set)
    write.csv(as.data.frame(variant.count.df), file.path(out.dir, "variant.counts.csv"))
    # mutation efficiency
    ...
    # browser()
    write.csv(data.frame(ref_start = crispr.set$insertion_sites$genomic_start
      , seq = crispr.set$insertion_sites$seq
      , cigar_label = crispr.set$insertion_sites$cigar_label
      , count = crispr.set$insertion_sites$count
      )
    , file.path(out.dir, "insertion.sites.csv")
    )
    write.csv(as.data.frame(consensusSeqs(crispr.set)), file.path(out.dir, "consensus.seqs.csv"))

    ### Frame Classification
    if(nrow(variant.count.df) > 0){

      # classify by frame
      variants.kinds.vec = rownames(variant.count.df)
      ...
      variants.count.df <- variant.count.df
...
SaveVariantsPlot <- function(crispr.set, out.dir){
  is.perfect.process = FALSE 
  tryCatch({
    if(is.null(crispr.set)){
      is.perfect.process = TRUE
      ...
      sorted.variant.count.vec <- variants.count.data[sort(variants.count.data[,1], decreasing = TRUE, index=T)$ix, ]
      # limit Top200
      if(length(sorted.variant.count.vec) > 200){
        min.freq <- sorted.variant.count.vec[201] / sum(sorted.variant.count.vec)
        limit.n <- 200
      }else{
        min.freq <- 0
        limit.n <- length(sorted.variant.count.vec)
      }
      align.sub.label <- "top200"
      align.plot.name <- paste("aligned.variants", align.sub.label, "pdf", sep=".")
      pdf(file.path(out.dir, align.plot.name), width = nchar(crispr.set$ref)/2.5, height = limit.n / 3 + 6)
      crispr.set.p <- plotVariants(crispr.set, row.ht.ratio = c(1, 6), col.wdth.ratio = c(8.5, 1)
        , plotAlignments.args = list(top.n = 200, min.insertion.freq = 0, max.insertion.size = 00, highlight.pam = FALSE, line.weight = 1
          ,legend.cols = 3, axis.text.size = 15, tile.height = 0.55, highlight.guide = FALSE, ins.size = 5, plot.text.size = 4, min.freq = min.freq)
        , plotFreqHeatmap.args = list(top.n = 200, legend.key.height = ggplot2::unit(1.5, "lines")
          , plot.text.size = 5, min.freq = min.freq)
      )
      dev.off()
      message(paste("Save alignment plot in ", file.path(out.dir, align.plot.name), sep=""))

      # make bar plot of variants
      # limit Top50
      sorted.variants.count.data <- variants.count.data
      sorted.variants.count.data[,1] <- sorted.variant.count.vec
      rownames(sorted.variants.count.data) <- names(sorted.variant.count.vec)
      if(length(sorted.variant.count.vec) > 50){
        selected.sorted.variants.count.data <- head(sorted.variants.count.data, n=51)
        fifty.below.sum.count <- sum(sorted.variant.count.vec[51:length(sorted.variant.count.vec)])
        selected.sorted.variants.count.data[51,] <- fifty.below.sum.count
        rownames(selected.sorted.variants.count.data)[51] <- "Others"
      }else{
        selected.sorted.variants.count.data <- head(sorted.variants.count.data, n=50)
      }
      barplot.sub.label <- "top50"
      
      variants.barplot.name <- paste("variants.barplot", barplot.sub.label, "pdf", sep=".")
      colPal2 <- colorRampPalette(c("white", "red", "yellow", "brown"))
      set.seed(1)
      pdf(file.path(out.dir, variants.barplot.name), width = 10, height = length(rownames(selected.sorted.variants.count.data)) / 12 + 4)
      selected.sorted.variants.count.data.p <- barplotAlleleFreqs(selected.sorted.variants.count.data, classify = FALSE, include.table = FALSE,
        , bar.colours = sample(colPal2(nrow(selected.sorted.variants.count.data))))
      plot(selected.sorted.variants.count.data.p)
      dev.off()
      message(paste("Save variants barplot in ", file.path(out.dir, variants.barplot.name), sep=""))
...
SaveKiIndelPlot <- function(correct.is.sn.list, reverse.is.sn.list, outdir, ki.type = c("left_ki", "right_ki"), nhej.insert.length, rc.nhej.insert.length){
  ...
  if(!is.null(aggregated.mutation.merge.size.table) & sum(aggregated.mutation.merge.size.table$Frequency) > 0){
    ...
    SavePieChart(aggregated.indels.size.class.count.table
      , c( "#B3D9C0", "#60A6A6", "#277A8C", "#F2EC9B", "#95D904", "#618C03", "#3E5902", "#C0D904" )
      , "Reads"
  ...
  if(!is.null(aggregated.mutation.merge.size.number.table) & sum(aggregated.mutation.merge.size.number.table$Frequency) > 0){
    ...
    SavePieChart(aggregated.merge.size.number.class.count.table
      , c( "#B3D9C0", "#60A6A6", "#277A8C", "#F2EC9B", "#95D904", "#618C03", "#3E5902", "#C0D904", "#BF7936", "#7F5124", "#F29F05" )
      , "Reads"
  ...
  if(!is.null(aggregated.mutation.merge.size.number.without.p_s0.table) & sum(aggregated.mutation.merge.size.number.without.p_s0.table$Frequency) > 0){
    ...
    SavePieChart(aggregated.merge.size.number.without.p_s0.class.count.table
      , c( "#B3D9C0", "#60A6A6", "#277A8C", "#F2EC9B", "#95D904", "#618C03", "#3E5902", "#C0D904", "#BF7936", "#7F5124" )
      , "Reads"
      , file.path(outdir, paste("[ⅱd]Rate_of_", ki.type, "_knock-in_substitution_and_indels_piechart(without_precise_ki).png", sep=""))
      , file.path(outdir, paste("[ⅱd]", ki.type, "_Rate_of_knock-in_substitution_and_indels(without_precise_ki).csv", sep=""))
    )
  }
}
