...
MakeCrisprsetList <- function(res.file.df, res.file.path){
  crisprset.list <- apply(res.file.df, MARGIN = 1, function(res.row){
    if(file.exists(file.path(res.row["aligner.res.dir.path"], res.file.path))){
      res <- list(readRDS(file.path(res.row["aligner.res.dir.path"], res.file.path))$cigar_freqs)
...
MakeCrisprsetListSimple <- function(res.file.df, res.file.path){
  crisprset.list <- apply(res.file.df, MARGIN = 1, function(res.row){
    if(file.exists(file.path(res.row["aligner.res.dir.path"], res.file.path))){
      res <- readRDS(file.path(res.row["aligner.res.dir.path"], res.file.path))$cigar_freqs
    }else{
      # message(paste0("Ignore ", row["sample.name"], " because ", file.path(row["aligner.res.dir.path"], res.file.path), " was not found."))
      res <- NULL
    }
...
CalcKLdivergence2 <- function(x.cigar_freqs, y.indel.data){
  ...
  MakePositionIndependentVariantsFreq <- function(cigar_freqs){
    ...
    rownames(cigar_freqs) <- gsub("[a-zA-Z0-9]*:", "", rownames(cigar_freqs))
    rownames(cigar_freqs) <- gsub(";;;", "M:", rownames(cigar_freqs))
    # SNV -> 1 base sub, 2 base sub, ...
    # InDel -> remove the position information
    aggregated.freq.table <- aggregate(reads ~ label, data = data.frame(label = rownames(cigar_freqs),  reads = cigar_freqs[,1]), sum)
    position.independent.cigar_freqs <- matrix(
      aggregated.freq.table$reads,
      ncol = 1
...
MakePreciseKnockinLengthVec <- function(res.file.df, wt.file.path, precise.knockin.file.path){
  knockin.length.vec <- apply(res.file.df, MARGIN = 1, function(res.row){
    if(file.exists(file.path(res.row["aligner.res.dir.path"], wt.file.path))){
      wt.dna <- readDNAStringSet(file.path(res.row["aligner.res.dir.path"], wt.file.path))[[1]]
      precise.knockin.dna <- readDNAStringSet(file.path(res.row["aligner.res.dir.path"], precise.knockin.file.path))[[1]]
...
MakeAlluvialDiagrams <- function(melt.df, color.vec, save.path.name, axis1.title, axis2.title, focus.type, unit = "kinds"){
  ...
  scale_y_continuous(breaks=seq(0, sum(melt.df$value), round(sum(melt.df$value)/10) + 1)) +
  ...
  ggsave(file = save.path.name, plot = alluvial.plot, dpi = 350, width = 10, height = 10)
  options(warn = 0)
}

MakeBumpChart <- function(all.rank.table, group.table, standard.label, label.vec, save.path.name){
  # remove variable having only one value
  n.unique.vec <- apply(all.rank.table[, label.vec], 2, function(x){length(unique(x))})
  available.ind.vec <- which(!(n.unique.vec == 1))
  available.label.vec <- label.vec[available.ind.vec]
  if(!(standard.label %in% available.label.vec)){
    message("The bump chart based on ",  standard.label, " cannot be generated.")
  }
  # make plot
  rank.table <- all.rank.table[, available.label.vec]
  ...
  options(warn = -1)
  bump.plot <- ggplot(malt.rank.table, aes(factor(Var2, level = available.label.vec), value,
    ...
    coord_cartesian(ylim = rev(c(1, nrow(rank.table)))) +
    labs(x = "", y = "Rank") +
    theme(legend.title = element_blank(), axis.text.x = element_text(angle = 90, hjust = 1), axis.text.y = element_text()) +
...
MakeFoldBoxPlot <- function(value.df, col.vec, is.negative.vec, ylim.vec, save.path.name){
  ...
  # Convert the reduction variable into 1/x
  value.df[,is.negative.vec] <- value.df[, is.negative.vec]^(-1)

  # If there is Inf, the element is converted into zero.
  for(col.ind in 1:ncol(value.df)){
    value.df[is.infinite(value.df[,col.ind]), col.ind] <- numeric(length(value.df[is.infinite(value.df[,col.ind]), col.ind]))
  }
...
MakeRateSumTable <- function(res.file.df, table.data.path, focus.sample.name.vec){
  # browser()
  table.list <- apply(res.file.df, MARGIN = 1, function(res.row){
    if(file.exists(file.path(res.row["aligner.res.dir.path"], table.data.path))){
      return(readRDS(file.path(res.row["aligner.res.dir.path"], table.data.path)))
    }else{
      return(NULL)
    }
  })
  ...
  if(!is.null(table.list)){
    names(table.list) <- table.name.vec
    focus.sample.first.ind <- which(names(table.list) %in% focus.sample.name.vec)[1]
    if(is.na(focus.sample.first.ind)){
      return(NULL)
    }
    # set window
    plot.window <- table.list[[1]][, 1]
    for(table in table.list){
      plot.window <- plot.window[plot.window %in% table[, 1]]
    }
    # sum values
    table.sum <- table.list[[focus.sample.first.ind]]
    table.sum <- table.sum[table.sum[, 1] %in% plot.window, ]
    table.sum$Frequency <- numeric(length(table.sum$Frequency))
    for(ind in 1:length(table.list)){
      if((names(table.list)[[ind]] %in% focus.sample.name.vec) & !is.null(table.list[[ind]]) & (sum(table.list[[ind]]$Frequency) > 0)){
        temp.table <- table.list[[ind]]
        temp.table <- temp.table[temp.table[, 1] %in% plot.window, ]
        table.sum$Frequency <- table.sum$Frequency +
          temp.table$Frequency / sum(temp.table$Frequency)
      }
    }
    return(table.sum)
  }else{
...
RunRateSumTableTest <- function(res.file.df, table.data.path, focus.sample.name.vec1, focus.sample.name.vec2){
  ...
  if(!is.null(table.list)){
    ...
    # collect the data of group1
    focus.sample.first.ind.1 <- which(names(table.list) %in% focus.sample.name.vec1)[1]
    if(is.na(focus.sample.first.ind.1)){
      return(NULL)
    }
    focus.sample.vec.1 <- numeric(0)
    for(ind in 1:length(table.list)){
      if(names(table.list)[[ind]] %in% focus.sample.name.vec1){
        temp.table <- table.list[[ind]]
        temp.table <- temp.table[temp.table[, 1] %in% plot.window, ]
        temp.freq.vec <- temp.table$Frequency
        if(sum(temp.freq.vec) > 0){
          temp.freq.vec <- temp.freq.vec / sum(temp.freq.vec)
        }
        focus.sample.vec.1 <- c(focus.sample.vec.1, temp.freq.vec)
      }
    }
    focus.sample.mat.1 <- matrix(focus.sample.vec.1, ncol=length(plot.window), byrow=TRUE)
    focus.sample.df.1 <- as.data.frame(focus.sample.mat.1, stringsAsFactors=FALSE)

    # collect the data of group2
    focus.sample.first.ind.2 <- which(names(table.list) %in% focus.sample.name.vec2)[1]
    if(is.na(focus.sample.first.ind.2)){
      return(NULL)
    }
    focus.sample.vec.2 <- numeric(0)
    for(ind in 1:length(table.list)){
      if(names(table.list)[[ind]] %in% focus.sample.name.vec2){
        temp.table <- table.list[[ind]]
        temp.table <- temp.table[temp.table[, 1] %in% plot.window, ]
        temp.freq.vec <- temp.table$Frequency
        if(sum(temp.freq.vec) > 0){
        ...
        focus.sample.vec.2 <- c(focus.sample.vec.2, temp.freq.vec)
      }
    }
    focus.sample.mat.2 <- matrix(focus.sample.vec.2, ncol=length(plot.window), byrow=TRUE)
    focus.sample.df.2 <- as.data.frame(focus.sample.mat.2, stringsAsFactors=FALSE)

    # t.test per x value (col)
    # browser()
    ...
    for(colind in 1:ncol(focus.sample.df.1)){
      ...
      t.test.res <- t.test(
        focus.sample.df.1[,colind],
        focus.sample.df.2[,colind]
      )
      n1 <- length(focus.sample.df.1[,colind])
      n2 <- length(focus.sample.df.2[,colind])
      sd1 <- sd(focus.sample.df.1[,colind])
      sd2 <- sd(focus.sample.df.2[,colind])
      mean1 <- mean(focus.sample.df.1[,colind])
      mean2 <- mean(focus.sample.df.2[,colind])
      cohend <- cohen.d(focus.sample.df.1[,colind], focus.sample.df.2[,colind])
      t.test.res.vec <- c(
        t.test.res.vec,
        c(as.character(n1),
          as.character(n2),
          ...
          as.character(mean1),
          as.character(mean2),
          as.character(t.test.res$statistic),
          as.character(t.test.res$parameter),
          as.character(t.test.res$p.value),
          as.character(cohend$estimate),
          as.character(cohend$magnitude)
          )
        )
    }
    t.test.res.mat <- matrix(t.test.res.vec, ncol=11, byrow=TRUE)
    colnames(t.test.res.mat) <- c("samplesize1", "samplesize2", "sd1", "sd2", "mean1", "mean2", "t", "df", "p-value", "effectsize", "magnitude")
    rownames(t.test.res.mat) <- plot.window

    return(t.test.res.mat)
  }else{
    # browser()
    return(NULL)
  }
  ...
  if(!is.null(table.list)){
    ...
    for(ind in 1:length(table.list)){
      if(names(table.list)[[ind]] %in% focus.sample.name.vec){
        if(sum(table.list[[ind]]$value) > 0){
          table.list[[ind]]$value <- table.list[[ind]]$value / sum(table.list[[ind]]$value)
        }else{
          table.list[[ind]]$value <- table.list[[ind]]$value
        }
    ...
    if(nrow(table.sum) == 0){
      zero.table <- data.frame(variable = rep(c("A", "T", "C", "G"), 20), value = numeric(80), trim.len = rep(1:20, each = 4))
      table.sum <- zero.table
    }
...
MakeGroupRateSumTable <- function(res.file.df, table.data.path, group.table, focus.variable, condition.label){
  ...
  if(!is.null(group.rate.sum.table)){
  ...
  # t-test
  group.t.test.table <- RunRateSumTableTest(
    res.file.df,
    table.data.path,
    MakeGroupSampleName(group.table, focus.variable, "High", condition.label),
    MakeGroupSampleName(group.table, focus.variable, "Low", condition.label)
  ...
  return(list(group.rate.sum.table, group.t.test.table))
}
...
SaveBarGroupPlot <- function(count.merge.table, file.name, xlab, ylab, group.name, width.margin){
  ...
  TestDistHighLow(
    count.merge.table["High",],
    count.merge.table["Low",],
    file.name
  )
}
...
SaveMultiBarGroupPlot <- function(count.merge.list, file.name, title, ylab, group.name, width.margin){
  ...
  }else if(length(unique(class.count.merge.table$Var1)) == 20){
    col.vec <- c("#3B7302", "#97BF04", "#6F7302", "#D9CC1E"
      ,"#009840", "#104277", "#25217D", "#005E6A"
      ,"#594011", "#8C7870", "#0D0D0D", "#401D1A"
      ,"#F28322", "#4557BF", "#400101", "#D93223"
      ,"#00931F", "#001298", "#000000", "#FE0000")
  }
  options(warn=-1)
  ...
  TestDistHighLow(
    count.merge.list$High$x,
    count.merge.list$Low$x,
    file.name
  )
}
...
TestDistHighLow <- function(high.vec, low.vec, file.name.excluding.ext){
  options(warn = -1)
  res <- ks.test(x=high.vec, y=low.vec)
  options(warn = 0)
  sink(paste(file.name.excluding.ext, "_kstest.txt"))
  print("Kolmogorov-Smirnov test")
  if(!is.null(res)){
    print(paste0("p-value : ", res$p.value))
  }else{
    print("Kolmogorov-Smirnov test was failed...")
  }
  sink()
  message(paste0("Save result of Kolmogorov-Smirnov test in ", paste(file.name.excluding.ext, "_kstest.txt")))
}
...
MakePositionRateSumTablePlots <- function(table.data.path, type, file.name.without.ext.vec){
  ...
  ############################ Condition1 Reads
  condition1.position.by.condition1.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition1.focus.variable)
  SaveTable(condition1.position.by.condition1.group.table[[1]], file.name.without.ext.vec[1])
  SaveTable(condition1.position.by.condition1.group.table[[2]], paste0(file.name.without.ext.vec[1], "_t-test"))
  SaveBarGroupPlot(
    condition1.position.by.condition1.group.table[[1]],
  ...
  if(analysis.mode %in% c(1, 4)){
    condition1.position.by.condition2.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition2.focus.variable)
    SaveTable(condition1.position.by.condition2.group.table[[1]], file.name.without.ext.vec[2])
    SaveTable(condition1.position.by.condition2.group.table[[2]], paste0(file.name.without.ext.vec[2], "_t-test"))
    SaveBarGroupPlot(
      condition1.position.by.condition2.group.table[[1]],
    ...
    condition1.position.by.enhancement.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(condition1.position.by.enhancement.group.table[[1]], file.name.without.ext.vec[3])
    SaveTable(condition1.position.by.enhancement.group.table[[2]], paste0(file.name.without.ext.vec[3], "_t-test"))
    SaveBarGroupPlot(
      condition1.position.by.enhancement.group.table[[1]],
    ...
    condition2.position.by.condition1.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition1.focus.variable)
    SaveTable(condition2.position.by.condition1.group.table[[1]], file.name.without.ext.vec[4])
    SaveTable(condition2.position.by.condition1.group.table[[2]], paste0(file.name.without.ext.vec[4], "_t-test"))
    SaveBarGroupPlot(
      condition2.position.by.condition1.group.table[[1]],
    ...
    condition2.position.by.condition2.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition2.focus.variable)
    SaveTable(condition2.position.by.condition2.group.table[[1]], file.name.without.ext.vec[5])
    SaveTable(condition2.position.by.condition2.group.table[[2]], paste0(file.name.without.ext.vec[5], "_t-test"))
    SaveBarGroupPlot(
      condition2.position.by.condition2.group.table[[1]],
    ...
    condition2.position.by.enhancement.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(condition2.position.by.enhancement.group.table[[1]], file.name.without.ext.vec[6])
    SaveTable(condition2.position.by.enhancement.group.table[[2]], paste0(file.name.without.ext.vec[6], "_t-test"))
    SaveBarGroupPlot(
      condition2.position.by.enhancement.group.table[[1]],
  ...
  ############################ Negative control Reads
  if(analysis.mode %in% c(1, 2)){
    
    negative.ctrl.position.by.condition1.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition1.focus.variable)
    SaveTable(negative.ctrl.position.by.condition1.group.table[[1]], file.name.without.ext.vec[7])
    SaveTable(negative.ctrl.position.by.condition1.group.table[[2]], paste0(file.name.without.ext.vec[7], "_t-test"))
    SaveBarGroupPlot(
      negative.ctrl.position.by.condition1.group.table[[1]],
      paste0(file.name.without.ext.vec[7], ".png"),
      paste0(type, " Start position (Direction : ->)"),
      "Cumulative ratio for negative control reads [labels]",
      "Condition1",
      0.1
    )
  }
  ...
  if(analysis.mode %in% c(1)){
    negative.ctrl.position.by.condition2.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition2.focus.variable)
    SaveTable(negative.ctrl.position.by.condition2.group.table[[1]], file.name.without.ext.vec[8])
    SaveTable(negative.ctrl.position.by.condition2.group.table[[2]], paste0(file.name.without.ext.vec[8], "_t-test"))
    SaveBarGroupPlot(
      negative.ctrl.position.by.condition2.group.table[[1]],
      paste0(file.name.without.ext.vec[8], ".png"),
      paste0(type, " Start position (Direction : ->)"),
      "Cumulative ratio for negative.control reads [labels]",
      "Condition2",
      0.1
    ...
    negative.ctrl.position.by.enhancement.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(negative.ctrl.position.by.enhancement.group.table[[1]], file.name.without.ext.vec[9])
    SaveTable(negative.ctrl.position.by.enhancement.group.table[[2]], paste0(file.name.without.ext.vec[9], "_t-test"))
    SaveBarGroupPlot(
      negative.ctrl.position.by.enhancement.group.table[[1]],
      paste0(file.name.without.ext.vec[9], ".png"),
      paste0(type, " Start position (Direction : ->)"),
      "Cumulative ratio for negative control reads [labels]",
      "Condition2/Conditon1 in precise knock-in",
      0.1
...
SavePeriodogramGroupPlot <- function(count.merge.table, is.negative.only = TRUE, file.name, group.name){
  ...
  if(is.null(ncol(non.zero.group.table))){ # When the row is 1, the dataframe is converted into vector, this pattern occurs.
    message("There is no data.")
    return()
  }
  if(ncol(non.zero.group.table) == 0){
    message("There is no data.")
    return()
  }
  ...
  options(warn = -1)
  merge.spec.table$class <- factor(merge.spec.table$class, levels=c("High","Medium","Low"))
...
MakeRateSumTablePlots <- function(table.data.path, x.axis.title, file.name.without.ext.vec, is.indel.size.analysis){
  ...
  ############################ Condition1 Reads
  condition1.indel.size.by.condition1.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition1.focus.variable)
  SaveTable(condition1.indel.size.by.condition1.group.table[[1]], file.name.without.ext.vec[1])
  SaveTable(condition1.indel.size.by.condition1.group.table[[2]], paste0(file.name.without.ext.vec[1], "_t-test"))
  SaveBarGroupPlot(
    condition1.indel.size.by.condition1.group.table[[1]],
    paste0(file.name.without.ext.vec[1], ".png"),
    x.axis.title,
  ...
  if(analysis.mode %in% c(1, 4)){
    condition1.indel.size.by.condition2.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition2.focus.variable)
    SaveTable(condition1.indel.size.by.condition2.group.table[[1]], file.name.without.ext.vec[2])
    SaveTable(condition1.indel.size.by.condition2.group.table[[2]], paste0(file.name.without.ext.vec[2], "_t-test"))
    SaveBarGroupPlot(
      condition1.indel.size.by.condition2.group.table[[1]],
      paste0(file.name.without.ext.vec[2], ".png"),
      x.axis.title,
      "Cumulative ratio for condition1 reads [reads]",
      "Condition2",
    ...
    condition1.indel.size.by.enhancement.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(condition1.indel.size.by.enhancement.group.table[[1]], file.name.without.ext.vec[3])
    SaveTable(condition1.indel.size.by.enhancement.group.table[[2]], paste0(file.name.without.ext.vec[3], "_t-test"))
    SaveBarGroupPlot(
      condition1.indel.size.by.enhancement.group.table[[1]],
      paste0(file.name.without.ext.vec[3], ".png"),
      x.axis.title,
      "Cumulative ratio for condition1 reads [reads]",
      "Condition2/Conditon1 in precise knock-in",
    ...
    condition2.indel.size.by.condition1.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition1.focus.variable)
    SaveTable(condition2.indel.size.by.condition1.group.table[[1]], file.name.without.ext.vec[4])
    SaveTable(condition2.indel.size.by.condition1.group.table[[2]], paste0(file.name.without.ext.vec[4], "_t-test"))
    SaveBarGroupPlot(
      condition2.indel.size.by.condition1.group.table[[1]],
      paste0(file.name.without.ext.vec[4], ".png"),
      x.axis.title,
    ...
    condition2.indel.size.by.condition2.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition2.focus.variable)
    SaveTable(condition2.indel.size.by.condition2.group.table[[1]], file.name.without.ext.vec[5])
    SaveTable(condition2.indel.size.by.condition2.group.table[[2]], paste0(file.name.without.ext.vec[5], "_t-test"))
    SaveBarGroupPlot(
      condition2.indel.size.by.condition2.group.table[[1]],
      paste0(file.name.without.ext.vec[5], ".png"),
      x.axis.title,
      "Cumulative ratio for condition2 reads [reads]",
      "Condition2",
    ...
    condition2.indel.size.by.enhancement.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(condition2.indel.size.by.enhancement.group.table[[1]], file.name.without.ext.vec[6])
    SaveTable(condition2.indel.size.by.enhancement.group.table[[2]], paste0(file.name.without.ext.vec[6], "_t-test"))
    SaveBarGroupPlot(
      condition2.indel.size.by.enhancement.group.table[[1]],
      paste0(file.name.without.ext.vec[6], ".png"),
      x.axis.title,
      "Cumulative ratio for condition2 reads [reads]",
      "Condition2/Conditon1 in precise knock-in",
  ...
  if(analysis.mode %in% c(1, 2)){
    ...
    negative.ctrl.indel.size.by.condition1.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition1.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.condition1.group.table[[1]], file.name.without.ext.vec[7])
    SaveTable(negative.ctrl.indel.size.by.condition1.group.table[[2]], paste0(file.name.without.ext.vec[7], "_t-test"))
    SaveBarGroupPlot(
      negative.ctrl.indel.size.by.condition1.group.table[[1]],
      paste0(file.name.without.ext.vec[7], ".png"),
      x.axis.title,
      "Cumulative ratio for negative control reads [reads]",
      "Condition1",
      0.1
    )
    SavePeriodogramGroupPlot(
      negative.ctrl.indel.size.by.condition1.group.table[[1]],
      is.indel.size.analysis,
      paste0(file.name.without.ext.vec[7], "_periodogram", ".png"),
      "Condition1"
    )
  }

  if(analysis.mode %in% c(1)){
    negative.ctrl.indel.size.by.condition2.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition2.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.condition2.group.table[[1]], file.name.without.ext.vec[8])
    SaveTable(negative.ctrl.indel.size.by.condition2.group.table[[2]], paste0(file.name.without.ext.vec[8], "_t-test"))
    SaveBarGroupPlot(
      negative.ctrl.indel.size.by.condition2.group.table[[1]],
      paste0(file.name.without.ext.vec[8], ".png"),
      x.axis.title,
      "Cumulative ratio for negative control reads [reads]",
      "Condition2",
    ...
    SavePeriodogramGroupPlot(
      negative.ctrl.indel.size.by.condition2.group.table[[1]],
      is.indel.size.analysis,
      paste0(file.name.without.ext.vec[8], "_periodogram", ".png"),
      "Condition2"
    )

    negative.ctrl.indel.size.by.enhancement.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.enhancement.group.table[[1]], file.name.without.ext.vec[9])
    SaveTable(negative.ctrl.indel.size.by.enhancement.group.table[[2]], paste0(file.name.without.ext.vec[9], "_t-test"))
    SaveBarGroupPlot(
      negative.ctrl.indel.size.by.enhancement.group.table[[1]],
      paste0(file.name.without.ext.vec[9], ".png"),
      x.axis.title,
      "Cumulative ratio for negative control reads [reads]",
      "Condition2/Conditon1 in precise knock-in",
    ...
    SavePeriodogramGroupPlot(
      negative.ctrl.indel.size.by.enhancement.group.table[[1]],
      is.indel.size.analysis,
      paste0(file.name.without.ext.vec[9], "_periodogram", ".png"),
      "Condition2/Conditon1 in precise knock-in"
    )
  }
...
MakeMaxSumClassTable <- function(res.file.df, table.data.path, focus.sample.name.vec){
  table.list <- apply(res.file.df, MARGIN = 1, function(res.row){
    ...
    #  ){
    if(file.exists(file.path(res.row["aligner.res.dir.path"], table.data.path))){
      if(sum(readRDS(file.path(res.row["aligner.res.dir.path"], table.data.path))$Frequency) > 0){ # total is non-zero.
        temp.table <- readRDS(file.path(res.row["aligner.res.dir.path"], table.data.path))
        ...
        message(paste0("Ignore ", res.row["sample.name"], " because each value is zero."))
        return(NULL)
      }
    #}else if(row["sample.label"] %in% c(condition1.label, condition2.label)){
    #  message(paste0("Ignore ", row["sample.name"], " because ", file.path(row["aligner.res.dir.path"], table.data.path), " was not found."))
...
MakeMaxSumInDelTablePlots <- function(table.data.path, file.name.without.ext.vec,
  ...
  if(type == "ej.indel.size.indel"){
    col.vec <- c("#8C4A32", "#F2ECD8", "#BF9E75", "#8C8304", "#D9CC14", "#D99ABC")
  }else if(type == "indel.size.indel"){
    # col.vec <- c("#F2ECD8", "#BF9E75", "#8C4A32", "#D9CC14", "#8C8304")
    col.vec <- c("#8C4A32", "#F2ECD8", "#BF9E75", "#8C8304", "#D9CC14")
  }else if(type == "indel.size.impreciseki"){
    col.vec <- c("#C0D904", "#B3D9C0", "#3E5902", "#277A8C", "#618C03", "#95D904", "#F2EC9B", "#60A6A6")
  ...
  # browser(expr = (type == "indel.size.impreciseki"))
  SavePieChart(
    MakeMaxSumClassTable(
      ...
      MakeGroupSampleName(
        ...
        focus.variable = condition1.focus.variable,
        group.name = "High")),
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          group.table = machiato.group.table,
          condition.label = condition1.label,
          focus.variable = condition2.focus.variable,
          group.name = "High")),
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          group.table = machiato.group.table,
          condition.label = condition1.label,
          focus.variable = enhancement.1.2.focus.variable,
          group.name = "High")),
  ...
  if(analysis.mode %in% c(1, 2)){
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          group.table = machiato.group.table,
          condition.label = negative.ctrl.label,
          focus.variable = condition1.focus.variable,
          group.name = "High")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[7], "_high_group.png")
    )
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition1.focus.variable,
          group.name = "Medium")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[7], "_medium_group.png")
    )
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition1.focus.variable,
          group.name = "Low")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[7], "_low_group.png")
    )
  }
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition2.focus.variable,
          group.name = "High")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[8], "_high_group.png")
    )
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition2.focus.variable,
          group.name = "Medium")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[8], "_medium_group.png")
    )
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition2.focus.variable,
          group.name = "Low")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[8], "_low_group.png")
    )
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = enhancement.1.2.focus.variable,
          group.name = "High")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[9], "_high_group.png")
    )
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = enhancement.1.2.focus.variable,
          group.name = "Medium")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[9], "_medium_group.png")
    )
    ...
    SavePieChart(
      MakeMaxSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = enhancement.1.2.focus.variable,
          group.name = "Low")),
      col.vec, "Genes", paste0(file.name.without.ext.vec[9], "_low_group.png")
    )
  }
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition1.focus.variable,
          group.name = "High")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[7], "_high_group.png")
    )
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition1.focus.variable,
          group.name = "Medium")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[7], "_medium_group.png")
    )
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition1.focus.variable,
          group.name = "Low")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[7], "_low_group.png")
    )
  }
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition2.focus.variable,
          group.name = "High")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[8], "_high_group.png")
    )
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition2.focus.variable,
          group.name = "Medium")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[8], "_medium_group.png")
    )
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = condition2.focus.variable,
          group.name = "Low")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[8], "_low_group.png")
    )
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = enhancement.1.2.focus.variable,
          group.name = "High")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[9], "_high_group.png")
    )
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = enhancement.1.2.focus.variable,
          group.name = "Medium")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[9], "_medium_group.png")
    )
    ...
    SavePieChart(
      MakeReadsSumClassTable(
        ...
        MakeGroupSampleName(
          ...
          focus.variable = enhancement.1.2.focus.variable,
          group.name = "Low")),
      col.vec, "Reads", paste0(file.name.without.ext.vec[9], "_low_group.png")
    )
  }
...
MakeScatterPlots <- function(table.data.path, x.axis.title, file.name.without.ext.vec){
  ...
  ############################ Condition1 Reads
  condition1.indel.size.by.condition1.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition1.focus.variable)
  ...
  if(analysis.mode %in% c(1, 4)){
    condition1.indel.size.by.condition2.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition2.focus.variable)
    ...
    condition1.indel.size.by.enhancement.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = enhancement.1.2.focus.variable)
    ...
    condition2.indel.size.by.condition1.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition1.focus.variable)
    ...
    condition2.indel.size.by.condition2.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition2.focus.variable)
    ...
    condition2.indel.size.by.enhancement.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = enhancement.1.2.focus.variable)
  ...
  if(analysis.mode %in% c(1, 2)){
    negative.ctrl.indel.size.by.condition1.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition1.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.condition1.group.table, file.name.without.ext.vec[7])
    SaveScatterGroupPlot(
      negative.ctrl.indel.size.by.condition1.group.table,
      paste0(file.name.without.ext.vec[7], ".png"),
      x.axis.title,
      "Condition1"
    )
  ...
  if(analysis.mode %in% c(1)){
    negative.ctrl.indel.size.by.condition2.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition2.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.condition2.group.table, file.name.without.ext.vec[8])
    SaveScatterGroupPlot(
      negative.ctrl.indel.size.by.condition2.group.table,
      paste0(file.name.without.ext.vec[8], ".png"),
      x.axis.title,
      "Condition2"
    )

    negative.ctrl.indel.size.by.enhancement.group.table <- MakeGroupAggregatedTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.enhancement.group.table, file.name.without.ext.vec[9])
    SaveScatterGroupPlot(
      negative.ctrl.indel.size.by.enhancement.group.table,
      paste0(file.name.without.ext.vec[9], ".png"),
      x.axis.title,
      "Condition2/Conditon1 in precise knock-in"
    )
...
MakeSeqRateSumTablePlots <- function(table.data.path, x.axis.title, y.axis.unit, file.name.without.ext.vec, width.margin.1, width.margin.2){
  ...
  ############################ Negative control Reads
  if(analysis.mode %in% c(1, 2)){
    # browser(expr = (table.data.path == "result/[2]machiato_local_alignment/Mixed/[Ⅱb]analysis_of_knock-in_junction/[ⅷa]Distribution_of_left_ki_MMEJ_Total_Microhomology_Sequence_Frequency.table.rds"))
    negative.ctrl.indel.size.by.condition1.group.table <- MakeGroupRateSumTable(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition1.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.condition1.group.table[[1]], file.name.without.ext.vec[7])
    SaveTable(negative.ctrl.indel.size.by.condition1.group.table[[2]], paste0(file.name.without.ext.vec[7], "_t-test"))
    SaveBarGroupPlot(
      ...
      paste0(file.name.without.ext.vec[7], ".png"),
      x.axis.title,
      paste0("Cumulative ratio for negative.ctrl reads [", y.axis.unit, "]"),
      "Condition1",
      width.margin.1
    )
  }
...
MakeSeqLengthRateSumTablePlots <- function(table.data.path, x.axis.title, y.axis.unit, file.name.without.ext.vec, width.margin.1, width.margin.2){
  ...
  ############################ Condition1 Reads
  condition1.indel.size.by.condition1.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition1.focus.variable)
  SaveTable(condition1.indel.size.by.condition1.group.table, file.name.without.ext.vec[1])
  ...
  if(analysis.mode %in% c(1, 4)){
    condition1.indel.size.by.condition2.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = condition2.focus.variable)
    SaveTable(condition1.indel.size.by.condition2.group.table, file.name.without.ext.vec[2])
    SaveMultiBarGroupPlot(
      condition1.indel.size.by.condition2.group.table,
      paste0(file.name.without.ext.vec[2], ".png"),
    ...
    condition1.indel.size.by.enhancement.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition1.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(condition1.indel.size.by.enhancement.group.table, file.name.without.ext.vec[3])
    SaveMultiBarGroupPlot(
      condition1.indel.size.by.enhancement.group.table,
      paste0(file.name.without.ext.vec[3], ".png"),
    ...
    condition2.indel.size.by.condition1.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition1.focus.variable)
    SaveTable(condition2.indel.size.by.condition1.group.table, file.name.without.ext.vec[4])
    SaveMultiBarGroupPlot(
      condition2.indel.size.by.condition1.group.table,
      paste0(file.name.without.ext.vec[4], ".png"),
    ...
    condition2.indel.size.by.condition2.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = condition2.focus.variable)
    SaveTable(condition2.indel.size.by.condition2.group.table, file.name.without.ext.vec[5])
    SaveMultiBarGroupPlot(
      condition2.indel.size.by.condition2.group.table,
      paste0(file.name.without.ext.vec[5], ".png"),
    ...
    condition2.indel.size.by.enhancement.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = condition2.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(condition2.indel.size.by.enhancement.group.table, file.name.without.ext.vec[6])
    SaveMultiBarGroupPlot(
      condition2.indel.size.by.enhancement.group.table,
      paste0(file.name.without.ext.vec[6], ".png"),
  ...
  if(analysis.mode %in% c(1, 2)){
    # browser(expr = x.axis.title == "Sequence Frequency in Imprecise Knock-In Class")
    negative.ctrl.indel.size.by.condition1.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition1.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.condition1.group.table, file.name.without.ext.vec[7])
    SaveMultiBarGroupPlot(
      negative.ctrl.indel.size.by.condition1.group.table,
      paste0(file.name.without.ext.vec[7], ".png"),
  ...
  if(analysis.mode %in% c(1)){
    negative.ctrl.indel.size.by.condition2.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = condition2.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.condition2.group.table, file.name.without.ext.vec[8])
    SaveMultiBarGroupPlot(
      negative.ctrl.indel.size.by.condition2.group.table,
      paste0(file.name.without.ext.vec[8], ".png"),
    ...
    negative.ctrl.indel.size.by.enhancement.group.table <- MakeGroupRateSumList(fltr.res.file.df, table.data.path, machiato.group.table, condition.label = negative.ctrl.label, focus.variable = enhancement.1.2.focus.variable)
    SaveTable(negative.ctrl.indel.size.by.enhancement.group.table, file.name.without.ext.vec[9])
    SaveMultiBarGroupPlot(
      negative.ctrl.indel.size.by.enhancement.group.table,
      paste0(file.name.without.ext.vec[9], ".png"),
...
# detected indel pattern vs detected indel pattern
MakeHeatMap2 <- function(detected.data1.list, detected.data2.list, y.axis.title, x.axis.title, file.name.excluding.ext, condition.label){
  
  # make matrix for plot
  
  detected.comp.kl.mat <- matrix(NA, ncol = length(detected.data1.list))
  sample.name.vec <- character(0)
  for(i in 1:length(detected.data1.list)){
    sample.name.vec <- c(sample.name.vec, names(detected.data1.list[[i]]))
  }
  colnames(detected.comp.kl.mat) <- sample.name.vec
  rownames(detected.comp.kl.mat) <- "temp"
  for(data1.ind in 1:ncol(detected.comp.kl.mat)){
    detected.comp.kl.vec <- numeric(0)
    machiato.data1.crisprset.name <- names(detected.data1.list[[data1.ind]])
    temp1.gene.name <- gsub(paste0("-", ".*", "$"), "", machiato.data1.crisprset.name)
    for(data2.ind in 1:ncol(detected.comp.kl.mat)){

      machiato.data2.crisprset.name <- names(detected.data2.list[[data2.ind]])
      temp2.gene.name <- gsub(paste0("-", ".*", "$"), "", machiato.data2.crisprset.name)

      # calc sym KL value
      detected.comp.kl.vec <- c(detected.comp.kl.vec, CalcKLdivergence(getElement(detected.data1.list[[data1.ind]], machiato.data1.crisprset.name), getElement(detected.data2.list[[data2.ind]], machiato.data2.crisprset.name)))
      if(length(detected.comp.kl.vec) < 2){ # initial value
        names(detected.comp.kl.vec) <- c(temp2.gene.name)
      }else{ # add value
        names(detected.comp.kl.vec) <- c(names(detected.comp.kl.vec)[1:length(detected.comp.kl.vec)-1], temp2.gene.name)
      }

    }
    detected.comp.kl.mat <- rbind(detected.comp.kl.mat, detected.comp.kl.vec)
    # just below sentence was written due to misunderstanding relationship between colnames and rownames.
    # actually, I should change colnames because colnames mean predicted score. but I changed rownames.
    # I need a few change to fix it. but it works correctly without dealing with it. So it remains.
    rownames(detected.comp.kl.mat) <- c(rownames(detected.comp.kl.mat)[1:nrow(detected.comp.kl.mat)-1], temp2.gene.name)
  }
  # rownames(detected.comp.kl.mat) # data2
  # colnames(detected.comp.kl.mat) # data1

  # TODO: re-modify the code to fit comparison between actual indels
  detected.comp.kl.mat <- detected.comp.kl.mat[-1, ]
  detected.comp.kl.mat <- detected.comp.kl.mat[, which(colnames(detected.comp.kl.mat) %in% paste0(rownames(detected.comp.kl.mat), "-", condition.label))]

  reorder.ind.vec <- unlist(sapply(paste0(rownames(detected.comp.kl.mat), "-", condition.label), function(x){
    return(which(colnames(detected.comp.kl.mat) %in% x))
  }))
  detected.comp.kl.mat <- detected.comp.kl.mat[, reorder.ind.vec]
  
  predicted.comp.kl.p.mat <- detected.comp.kl.mat # just renamed same matrix
  colnames(predicted.comp.kl.p.mat) <- gsub(paste0("-", condition.label, "$"), "", colnames(predicted.comp.kl.p.mat))
  ...
  match.value.vec <- numeric(0)
  nonmatch.value.vec <- numeric(0)
  for(row.ind in 1:nrow(detected.comp.kl.mat)){
    for(col.ind in 1:ncol(detected.comp.kl.mat)){
      if(row.ind == col.ind){
        match.value.vec <- c(match.value.vec, log2(detected.comp.kl.mat)[row.ind, col.ind])
      }else{
        nonmatch.value.vec <- c(nonmatch.value.vec, log2(detected.comp.kl.mat)[row.ind, col.ind])
      }
    }
  }
...
MakeAlluvialDiagramsCRISPRessoMaChIAto <- function(){
  ...
  all.dataframe.reads.list <- apply(fltr.res.file.df, MARGIN = 1, function(res.row){
    if(file.exists(file.path(res.row["classifier.res.dir.path"], "ALL_dataframe.csv"))){
      temp.all.df <- read.csv(file.path(res.row["classifier.res.dir.path"], "ALL_dataframe.csv"), header = TRUE)
      part.name.vec <- c(
        "NHEJ",
        "UNMODIFIED",
        "HDR",
      ...
      temp.part.df <- rbind(temp.part.df, data.frame(
        NHEJ=c(
        ...
        X.Reads=numeric(20),
        CRISPResso_reclassification_labels=c(
          "HDR", "HDR", "HDR", "HDR",
          "Mixed HDR-NHEJ", "Mixed HDR-NHEJ", "Mixed HDR-NHEJ", "Mixed HDR-NHEJ",
          "NHEJ", "NHEJ", "NHEJ", "NHEJ",
  ...
  all.dataframe.variants.list <- apply(fltr.res.file.df, MARGIN = 1, function(res.row){
    if(file.exists(file.path(res.row["classifier.res.dir.path"], "ALL_dataframe.csv"))){
      temp.all.df <- read.csv(file.path(res.row["classifier.res.dir.path"], "ALL_dataframe.csv"), header = TRUE)
  ...
  SaveTable(total.all.dataframe.variantcnt.melt.table, file.path(comparison.analysis.dir, "[g1]Alluvial_Diagrams_of_CRISPResso_MaChIAto_Variants"))
  MakeAlluvialDiagrams(total.all.dataframe.variantcnt.melt.table,
    c("#F2E2CE", "#548C1C", "#A60321", "#F20C36","gray50", "#F2E2CE", "#548C1C", "#A60321", "#F20C36"),
    file.path(comparison.analysis.dir, "[g1]Alluvial_Diagrams_of_CRISPResso_MaChIAto_Variants.png"),
  ...
  SaveTable(total.all.dataframe.readcnt.melt.table, file.path(comparison.analysis.dir, "[g2]Alluvial_Diagrams_of_CRISPResso_MaChIAto_Reads"))
  MakeAlluvialDiagrams(total.all.dataframe.readcnt.melt.table,
    c("#F2E2CE", "#548C1C", "#A60321", "#F20C36","gray50", "#F2E2CE", "#548C1C", "#A60321", "#F20C36"),
    file.path(comparison.analysis.dir, "[g2]Alluvial_Diagrams_of_CRISPResso_MaChIAto_Reads.png"),
    "CRISPResso Classification", "MaChIAto Classification", "all allele",
    "reads")
}