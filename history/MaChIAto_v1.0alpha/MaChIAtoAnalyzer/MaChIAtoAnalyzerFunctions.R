...
SaveLabeledGroupPlot <- function(value.set, annotation.set, response, figure.dir, ki_type){
  plot.df <- data.frame(group.name = value.set[, "group.name"]
      , sample.type = character(length(value.set[, "group.name"]))
      , Value = value.set[, response]
      , Levels = as.factor(annotation.set[, response])
  )
  pdf(file.path(figure.dir, paste0(ki_type, "_", response, "_boxplot.pdf")), width = 8, height = 8)
  p <- ggplot(plot.df, aes(x=sample.type, y=Value)) +
      geom_boxplot(fill = "#FFFFFF", color = "black", alpha = 0.75, show.legend = FALSE, outlier.shape = NA) + 
      geom_jitter(width = 0.25, aes(colour=Levels), size = 3, alpha = 0.75) +
      ggtitle(response) +
      geom_text(aes(label = group.name), size = 3) +
      scale_color_manual(values = c("#60FF4A", "#FF1931"))
  plot(p)
  dev.off()
}
...
CalcMHProp <- function(microhomology.seqset, focus.length, scaffoldset, annotationset, is.left.microhomology.logical, out.align.file, out.prop.file){
    if(!file.exists(file.path(output.dir, out.prop.file))){
        ...
        prop.info.CalcMicroHomologyPropPK <- CalcMicroHomologyPropPK$new(
            ...
            #align.range.vec = 40:nchar(scaffoldset[1]),
            #align.range.vec = 5:focus.length,
...
PlotTopRankScattter = function(future.data.df, response.continuous.df, rank.vec, min.rank, sample.type){
    ...
    for(rank in 1:min.rank){
        top.future.name <- names(rank.vec[1:min.rank])[rank]
