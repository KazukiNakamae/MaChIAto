...
if(untreated.label != ""){ # untreated label is entered.
  is.conservative.sample.vec <- apply(res.file.df, MARGIN = 1, function(row){
    ...
    all.df <- read.csv(file.path(row["classifier.res.dir.path"], "ALL_dataframe.csv"), header = TRUE)
    crispresso.unmodified.df <- filter(all.df, CRISPResso_reclassification_labels == "Unmodified")
...
if(untreated.label != "" & negative.ctrl.label != ""){ # untreated label is entered.
  for(focus.sample.target in unique(res.file.df$sample.target)){
    ...
    temp.untreated.crispresso.unmodified.df <- filter(temp.untreated.all.table, CRISPResso_reclassification_labels == "Unmodified")
    ...
    temp.negative.ctrl.crispresso.unmodified.df <- filter(temp.negative.ctrl.all.table, CRISPResso_reclassification_labels == "Unmodified")
...
if(condition1.label != "" & condition2.label != ""){ # untreated label is entered.
  for(focus.sample.target in unique(res.file.df$sample.target)){
    ...
    temp.condition1.crispresso.hdr.df <- filter(temp.condition1.all.table, CRISPResso_reclassification_labels == "HDR")
    ...
    temp.condition2.crispresso.hdr.df <- filter(temp.condition2.all.table, CRISPResso_reclassification_labels == "HDR")
